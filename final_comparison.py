...
def compare_files():
    ...
    # Find common tickers and compare
    common_tickers = set(source_data.keys()) & set(our_data.keys())
    print(f"Common positions: {len(common_tickers)}")
    ...
    for ticker in sorted(common_tickers):
        source_row = source_data[ticker]
        our_row = our_data[ticker]
